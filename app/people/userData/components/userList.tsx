...
const Rendering = ({ friendList }: { friendList: any }) => {
  console.log("Rendering : ", friendList);
  const setFriendList = useSetRecoilState(Friend);
  ...
  return friendList.map((data: any, index: number) => (
    <div key={index}>
...
export function UserList({ friendList }: { friendList: any }) {
  console.log("userList : ", friendList.length);

  return <Rendering friendList={friendList} />;
}
