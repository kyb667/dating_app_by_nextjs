...
import { PopulateFriend } from "@/state/friend/PopulateFriend";
// db
import { populatePerson, createRelation } from "@/lib/axios";
// config
...
const FriendDataComponentsHeader = ({
  ...
  const [populareFriendList, setPopulareFriendList] =
    useRecoilState(PopulateFriend);
  ...
  const clickLoveBtn = async ({ friendId }: { friendId: string }) => {
    ...
    if (res === CONFIG.STATUS_CODE.SUCCESS) {
      let data: any = [...populareFriendList];
      data[index] = { ...data[index], love: true };
      setPopulareFriendList(data);
...
const LoginCheck = () => {
  ...
  const getData = async () => {
    try {
      const res = await populatePerson();
      console.log(res);
      if (res.code === CONFIG.STATUS_CODE.SUCCESS) {
        setPopulareFriendList(res.data);
      } else {
        alert("error");
      }
    } catch (error: any) {
